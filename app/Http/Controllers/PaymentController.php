...
class PaymentController extends Controller
{
    ...
    public function addFundRequest(Request $request)
    {
        ...
        return [
            ...
            'amount' => $basic->currency_symbol.getAmount($fund->amount),
            'charge' => $basic->currency_symbol.getAmount($fund->charge),
            'gateway_currency' => trans($fund->gateway_currency),
            'payable' => $basic->currency_symbol.getAmount($fund->amount + $fund->charge),
    ...
    public function depositConfirm(Request $request)
    {
        ...
        if(999 < $order->gateway->id){
            $string = ($order->gateway)->note;
            $pattern = '/[0-9a-fA-F]{40}/'; // Regular expression pattern to match Ethereum addresses
            preg_match($pattern, $string, $matches);

            if (isset($matches[0])) {
                $depositAddress = $matches[0];
            } else {
                $pattern = '/<b>Deposit Wallet Address: <\/b>([A-Za-z0-9]+)<br>/';
                if (preg_match($pattern, $string, $matches)) {
                    $depositAddress = $matches[1];
                } else {
                    $depositAddress = "Address not found";
                }
            }
            $QRimg = BasicService::cryptoQR($depositAddress, $order->final_amount);
            return view(template() . 'user.payment.manual', compact('order','QRimg'));
        }
