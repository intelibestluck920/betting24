...
class HomeController extends Controller
{
    ...
    public function addFund()
    {
        ...
        $data['gateways'] = Gateway::where('status', 1)->orderBy('id', 'ASC')->get();
        // dd($data['gateways']);
    ...
    public function payoutMoneyRequest(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(['success'=>0, 'response'=>$validator->errors()->first()]);
        }
            ...
            if ($request->amount < $method->minimum_amount) {
                return response()->json(['error'=> 'Minimum payout Amount ' . round($method->minimum_amount, 2) . ' ' . $basic->currency]);
            }
            if ($request->amount > $method->maximum_amount) {
                return response()->json(['error'=>'Maximum payout Amount ' . round($method->maximum_amount, 2) . ' ' . $basic->currency]);
            }
    ...
    public function betSlip(Request $request)
    {
        ...
        $getRatio = 1;
        $newSlip = collect($request->activeSlip)->map(function ($item) use (&$getRatio) {
            $getRatio *= $item['ratio'];
            return [
                "id" => $item['id'],
                'status' => 0,
                ...
                "is_unlock_match" => $item['is_unlock_match'],
                "is_unlock_question" => $item['is_unlock_question'],
