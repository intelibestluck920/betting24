                ...
                <div class="col-md-12">
                    <div class="card secbg br-4">
                        <div class="card-body ">
                            <div class="row ">
                                <div class="col-md-12">
                                    <h3 class="title text-center font_class">{{trans('Please follow the instruction below')}}</h3>
                                    <p class="text-center mt-2 font_class">{{trans('You have requested to deposit')}}  <b class="text--base">{{getAmount($order->amount)}}
                                            {{$basic->currency}}</b>, {{trans('please pay exactly')}}
                                        <b class="text--base font_class">{{getAmount($order->final_amount)}} {{$order->gateway_currency}}</b>  {{trans(' to the address below for successful deposit.')}}
                                    </p>
                                    <center>
                                    <p class=" mt-2 font_class">
                                        <?php echo ($order->gateway)->note; ?>
                                    </p>
                                    <img src="{{$QRimg}}" class="cstm_scan_img">
                                    <form action="" method="post" enctype="multipart/form-data"
                                        ...
                                        @if(optional($order->gateway)->parameters)
                                            @foreach($order->gateway->parameters as $k => $v)
                                                @if($v->type == "text")
                                                    <div class="col-md-6 mt-2 font_class">
                                        ...
                                        <div class="col-md-6 ">
                                            <div class=" form-group">
                                                <button type="submit" class="btn-custom w-100 mt-3">
                                                    <span class="font_class">@lang('Confirm Now')</span>
                                                </button>
                                            </div>
                                        </div>
                                    </form>
